...
#
# build_format_manifest.py - Transcribe a reviewed FORMAT PRG into the
# checked-in hex manifest that CMake ships. Single-file twin of
# scripts/build_banner_manifest.py.
#
# WHY THIS IS NOT A BUILD STEP
# ----------------------------
# FORMAT is assembled by the NATIVE CASM assembler running on the C64 -- it
...
# reviewed manifest (src/external/format/format.ref.hex) which
...
# record (src/external/format/format-derivation.md) -- never a build step.
#
...
# src/external/format/:
#   format.s        the source
#   FORMAT_VERSION  app version "MAJOR.MINOR.STAGE" (feeds the generated
#                  formatver.s via scripts/gen_format_version.py)
#   BUILD_FORMAT    the build counter (feeds formatver.s too)
# All three are recorded as source_sha256 entries so hex_manifest_to_bin.py's
# --source-dir check hard-fails the build if any changes without a manifest
# regeneration. (formatver.s itself is a build product, never checked in;
# its content is fully determined by FORMAT_VERSION + BUILD_FORMAT + the
# generator script, which is reviewed like any code.)
...
DEFAULT_MANIFEST = REPO_ROOT / "src" / "external" / "format" / "format.ref.hex"
DEFAULT_SOURCE_DIR = REPO_ROOT / "src" / "external" / "format"

FORMAT_SOURCE_NAMES = ("format.s", "FORMAT_VERSION", "BUILD_FORMAT")


def fail(msg):
    sys.stderr.write(f"build_format_manifest.py: error: {msg}\n")
    sys.exit(1)


def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Transcribe a reviewed FORMAT PRG into its hex manifest.")
    ...
    ap.add_argument("--source-dir", type=Path, default=DEFAULT_SOURCE_DIR,
                    help="directory holding format.s / FORMAT_VERSION / "
                         "BUILD_FORMAT this PRG was assembled from "
                         "(default: %(default)s)")
    ...
    source_shas = []
    for name in FORMAT_SOURCE_NAMES:
        p = args.source_dir / name
        if not p.is_file():
            fail(f"{p}: not a file (required to record its source_sha256 -- "
                 "pass --source-dir to point at the FORMAT sources this PRG "
                 "was actually assembled from)")
        source_shas.append((name, hashlib.sha256(p.read_bytes()).hexdigest()))

    lines = [
        "# FORMAT -- reviewed hex manifest",
        "#",
        "# These are the bytes that ship as FORMAT.PRG. They are transcribed",
        ...
        "# derivation record at src/external/format/format-derivation.md",
