...
#
# build_label_manifest.py - Transcribe a reviewed LABEL PRG into the
# checked-in hex manifest that CMake ships. Single-file twin of
# scripts/build_banner_manifest.py.
#
# WHY THIS IS NOT A BUILD STEP
# ----------------------------
# LABEL is assembled by the NATIVE CASM assembler running on the C64 -- it
...
# reviewed manifest (src/external/label/label.ref.hex) which
...
# record (src/external/label/label-derivation.md) -- never a build step.
#
...
# src/external/label/:
#   label.s        the source
#   LABEL_VERSION  app version "MAJOR.MINOR.STAGE" (feeds the generated
#                  labelver.s via scripts/gen_label_version.py)
#   BUILD_LABEL    the build counter (feeds labelver.s too)
...
# regeneration. (labelver.s itself is a build product, never checked in;
# its content is fully determined by LABEL_VERSION + BUILD_LABEL + the
...
DEFAULT_MANIFEST = REPO_ROOT / "src" / "external" / "label" / "label.ref.hex"
DEFAULT_SOURCE_DIR = REPO_ROOT / "src" / "external" / "label"

LABEL_SOURCE_NAMES = ("label.s", "LABEL_VERSION", "BUILD_LABEL")


def fail(msg):
    sys.stderr.write(f"build_label_manifest.py: error: {msg}\n")
    sys.exit(1)


def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Transcribe a reviewed LABEL PRG into its hex manifest.")
    ...
    ap.add_argument("--source-dir", type=Path, default=DEFAULT_SOURCE_DIR,
                    help="directory holding label.s / LABEL_VERSION / "
                         "BUILD_LABEL this PRG was assembled from "
                         "(default: %(default)s)")
    ...
    for name in LABEL_SOURCE_NAMES:
        ...
        if not p.is_file():
            fail(f"{p}: not a file (required to record its source_sha256 -- "
                 "pass --source-dir to point at the LABEL sources this PRG "
                 "was actually assembled from)")
        source_shas.append((name, hashlib.sha256(p.read_bytes()).hexdigest()))

    lines = [
        "# LABEL -- reviewed hex manifest",
        "#",
        "# These are the bytes that ship as LABEL.PRG. They are transcribed",
        ...
        "# derivation record at src/external/label/label-derivation.md",
