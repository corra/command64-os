...
- **CONWAY → CASM-native (2026-09-03)** — fourth external-application
  migration from the CASM-native viability review, and the first
  **multi-module** one. `CONWAY`'s ca65/ld65 build is retired; `conway.prg`
  now ships from a reviewed hex manifest
  (`src/external/conway/conway.ref.hex`). The two ca65 objects are
  flattened into one CASM `.INCLUDE` chain: `conway.s`
  (`conway_main.s` renamed, constants inline, `@local` labels) →
  `conwayver.s` → `conwaymenu.s` → `conwaygrid.s` (`conway_grid.s` renamed
  — no underscore, which CASM's `.INCLUDE` filename lookup can't match on a
  cc1541 disk). The menu/status **screen-code** text (ca65 `.CHARMAP`
  macros) is now generated as explicit `.BYTE` data by
  `scripts/gen_conway_menu.py`; the ~23 compile-time `.assert` layout
  guards move host-side to `scripts/check_conway_layout.py`. Version data
  (`EXITBANNER` + `MENUVERSION`, `CONWAY v0.4.1.1063`) generated by
  `scripts/gen_conway_version.py`. **No behaviour change** — the
  CASM-native image is byte-identical (0 differences across all 4288 image
  bytes) to an independent same-base (`$3400`) ca65/ld65 build of the same
  four sources; the 182-entry R6 table matches the retired ca65 build's
  count and verifies at `$3800`/`$5000`/`$9000` with `grid0`/`grid1`
  staying page-aligned. Live `COMP CNW.PRG CONWAY.REF` → `FILES COMPARE
  OK`, plus a full menu / preset / rule-edit / pause / clear / simulation
  / exit sweep. New: `scripts/gen_conway_version.py`,
  `scripts/gen_conway_menu.py`, `scripts/check_conway_layout.py`,
  `scripts/build_conway_manifest.py`,
  `src/external/conway/conway-derivation.md`, `command64_conway_test_d64`.
