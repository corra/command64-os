...
- **DASH WP8 VMM Test Page**: `dvmm.s` implements a repeatable REU self-test (Unavailable/Ready/Running/Passed/Failed/Cleanup-failed states) — one 4KB allocation, three patterns across sixteen 256-byte blocks, unconditional single-free-attempt cleanup on every path, gated `T`/`t` trigger on the VMM page only.
- **DASH WP7 Applications Page**: `dapp.s` renders all 16 public application-table slots (name/range/size/`U`/`R`/`V`/`S` flags) from `DOS_GET_APP_INFO` alone, bounded name width, no private app-table access.
