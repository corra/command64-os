...
;
; CASM listing capture and serialization: two conditional VMM stores (a fixed
; 16-byte metadata record per captured physical source line, and a mirror of
; every Pass 2 source-generated byte), deterministic replay/formatting, and
; committed-aware `.LST` ownership. WP54 wired `/L` into production after the
; PRG commit; WP59 hardens failure, retry, and metadata-validation contracts.
;
...
; integration added the first capture call sites in WP51 increment 5.
...
; Retained legacy module ABI. No external consumer currently needs the open-name
; buffer, but removing an export is outside WP59 Increment 8's no-ABI-change audit.
...
; no-op on the normal non-`/L` path.
;
...
; SHORT_WRITE ($3D-$41). WP54 subsequently wired this lifecycle into the real
; Pass 2 tail in the required order: the PRG is committed before any listing
...
; CasmListFileOpened. Production `artifactsAbort` calls it on fatal paths after
; WP54 integration.
;
...
;            of range for its own kind, or included catalog metadata has a
;            device outside CASM_DEVICE_MIN..CASM_DEVICE_MAX) or
;            CASM_DIAG_VMM_TRANSFER_FAILED (propagated from a rejected
;            includeCatalogRead). Invalid device metadata produces no
;            consumable resolved-name output and never indexes the device
;            string tables.
; Preserves: none
...
;
; Production casm.s calls it from the real Pass 2 tail after outputCommit.
; =============================================================================
