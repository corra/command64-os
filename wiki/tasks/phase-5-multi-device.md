...
- [x] Add support for dynamically changing file open routines to use `CurrentDevice`.
- [x] Update `path.asm` and `loader.asm` to target the active device.
- [ ] Update shell prompt to display the active device number — see [phase-5-prompt-device.md](phase-5-prompt-device.md).
