...
_The ld65 linker combines object files into an executable file. ld65 is highly configurable and uses configuration files for high flexibility._
...
## 3. [Search paths](#3-search-paths)
  ...
  * 3.1 [Library search path](#31-library-search-path)
  * 3.2 [Object file search path](#32-object-file-search-path)
  * 3.3 [Config file search path](#33-config-file-search-path)


## 4. [Detailed workings](#4-detailed-workings)

## 5. [Configuration files](#5-configuration-files)

  * 5.1 [Memory areas](#51-memory-areas)
  * 5.2 [Segments](#52-segments)
  * 5.3 [Output files](#53-output-files)
  * 5.4 [OVERWRITE segments](#54-overwrite-segments)
  * 5.5 [LOAD and RUN addresses (ROMable code)](#55-load-and-run-addresses-romable-code)
  * 5.6 [Other MEMORY area attributes](#56-other-memory-area-attributes)
  * 5.7 [Other SEGMENT attributes](#57-other-segment-attributes)
  * 5.8 [The FILES section](#58-the-files-section)
  * 5.9 [The FORMAT section](#59-the-format-section)
  * 5.10 [The FEATURES section](#510-the-features-section)
  * 5.11 [The SYMBOLS section](#511-the-symbols-section)


## 6. [Special segments](#6-special-segments)

  * 6.1 [INIT](#61-init)
  * 6.2 [LOWCODE](#62-lowcode)
  * 6.3 [ONCE](#63-once)
  * 6.4 [STARTUP](#64-startup)
  * 6.5 [ZPSAVE](#65-zpsave)


## 7. [Debug Info](#7-debug-info)

  * 7.1 [Libraries](#71-libraries)
  * 7.2 [Lines](#72-lines)
  * 7.3 [Modules](#73-modules)
  * 7.4 [Scopes](#74-scopes)
  * 7.5 [Segment Info](#75-segment-info)
  * 7.6 [Source files](#76-source-files)
  * 7.7 [Spans](#77-spans)
  * 7.8 [Symbols](#78-symbols)
  * 7.9 [Types](#79-types)
...
The -o switch is used to give the name of the default output file. Depending on your output configuration, this name _might not_ be used as the name for the output file. However, for the default configurations, this name is used for the output file name.
...
This option allows you to create a file that contains all global labels and may be loaded into the VICE emulator using the `ll` (load label) command or into the Oricutron emulator using the `sl` (symbols load) command. You may use this to debug your code with VICE. Note: Older versions had some bugs in the label code. If you have problems, please get the latest [VICE](http://vice-emu.sourceforge.net) version.
...
That procedure (parsing and reading from left to right) does mean that a library may only satisfy references for object modules (given directly or from a library) named _before_ that library. With the command line
...
Case is ignored for keywords, that is, section or attribute names, but it is _not_ ignored for names and strings.
...
The syntax above is used in all sections of the config file. The name (`ROM1` etc.) is said to be an identifier, the remaining tokens up to the semicolon specify attributes for this identifier. You may use the equal sign to assign values to attributes, and you may use a comma to separate attributes, you may also leave both out. But you _must_ use a semicolon to mark the end of the attributes for one identifier. The section above may also have looked like this:
...
There are of course more attributes for a memory section than just start and size. Start and size are mandatory attributes, that means, each memory area defined _must_ have these attributes given (the linker will check that). I will cover other attributes later. As you may have noticed, I've used a comment in the example above. Comments start with a hash mark ('#'), the remainder of the line is ignored if this character is found.
...
There are two other available formats, one is the o65 format specified by Andre Fachat (see the [6502 binary relocation format specification](http://www.6502.org/users/andre/o65/fileformat.html)). It is defined like this:
...
The `FORMAT` section is used to describe file formats. The default (binary) format has currently no attributes, so, while it may be listed in this section, the attribute list is empty. The second supported format, [o65](http://www.6502.org/users/andre/o65/fileformat.html), has several attributes that may be defined here.
...
Specify a symbol as the run address of the binary, the loader will call this address after all the file is loaded in memory. If the attribute is omitted, no run address is included in the file.

**`INITAD = memory_area : symbol`**

Specify a symbol as the initialization address for the given memory area. The binary loader will call this address just after the memory area is loaded into memory, before continuing loading the rest of the file.
...
This is an optional attribute. If specified, an additional symbol is defined by the linker using the given name. The value of this symbol is the number of entries (_not_ bytes) in the table. While this attribute is optional, it is often useful to define it.
...
For more information, see the `.CONDES` command in the [ca65 manual](ca65.html).
...
Please note that order is important: The default start address must be defined _before_ the `%S` symbol is used in the config file. This does usually mean, that the `FEATURES` section has to go to the top of the config file.
